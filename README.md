...
- **Performance Optimized**: 
  - Granular state subscriptions to minimize rerenders
  - Memoized components for optimal rendering
  - Validation state computed only when status changes (invalid ↔ valid)
  - Each step link only rerenders when its own validation status changes
...
│   │       │   └── constants.ts         # Path constants and step definitions
│   │       ├── hooks/
│   │       │   └── hooks.ts             # Custom hooks (prefill functionality)
│   │       ├── queries/
│   │       │   └── queries.ts          # React Query hooks
│   │       ├── store/
│   │       │   └── onboardingStore.ts   # Zustand store for state management
│   │       ├── steps/
...
│   ├── test/
│   │   ├── mocks.tsx                   # Test mocks and utilities
...
├── .cursor/
│   └── rules/                          # Cursor IDE rules (modular structure)
├── .github/
│   └── workflows/
...
- **Debounced input**: 400ms delay to reduce unnecessary API calls
- **Request cancellation**: Older requests are automatically cancelled when new ones are fired
- **Race condition prevention**: Only the result of the last fired request is used to update form state
...
- Form state is managed through **Zustand** for optimal performance
...
- Path constants are centralized in `config/constants.ts` for maintainability
- React Query is configured with optimized defaults (5min staleTime, disabled refetch on window focus)

## Performance Optimizations

The application is optimized for minimal rerenders and optimal performance:

### State Management (Zustand)
- **Granular selectors**: Components subscribe only to the specific state slices they need
- **Computed validation state**: Validation status is computed in the store and only updates when status changes (invalid ↔ valid)
- **Selective updates**: Store setters only update `stepValidation` when the validation status actually changes

### Component Optimization
- **Memoized components**: `StepperNav` and `StepLink` components are memoized to prevent unnecessary rerenders
- **Individual subscriptions**: Each step link subscribes to its own validation field, so only the relevant link rerenders when its validation changes
- **Stable props**: Props are memoized and compared efficiently to prevent parent rerenders

### Request Optimization
- **Debounced prefill**: Email prefill requests are debounced by 400ms
- **Request cancellation**: Older prefill requests are cancelled when new ones are fired
- **React Query caching**: Countries data is cached with 5-minute stale time

### Result
- Typing in any input field only rerenders the relevant step component
- Step links in the header only rerender when their specific validation status changes
- Footer only rerenders when the current step's validation status changes
- No unnecessary rerenders of navigation components during form input

## Testing

The project includes comprehensive tests for all step components:
...
Tests are located in `src/components/onboarding-wizard/steps/__tests__/` and use:
